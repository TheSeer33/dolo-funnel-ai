...
import { Plus, BarChart2, Zap, LogOut, Eye, Trash2, ExternalLink } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const avgConvRate = funnels.length > 0 ? ((totalConversions / Math.max(totalViews, 1)) * 100).toFixed(1) : '0.0'
    ...
    <div className="min-h-screen bg-black text-white">
      <header className="border-b border-white/10 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={signOut} className="text-white/40 hover:text-white transition-colors text-sm">Sign out</button>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            { label: 'Avg. Conversion', value: `${avgConvRate}%`, icon: BarChart2 },
          ].map((stat, i) => (
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {funnels.map(funnel => <FunnelCard key={funnel.id} funnel={funnel} onDelete={() => deleteFunnel(funnel.id)} />)}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-colors">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`text-xs px-2 py-1 rounded-full capitalize ${statusColors[funnel.status] || ''}`}>{funnel.status}</span>
      </div>
